...
- **主題**：資訊視覺化與圖標設計
- **工具**：AI Icon 生成 + Affinity Designer (向量化)
- **重點方向**：
    - **資訊簡化**：將複雜的文字流程轉化為直觀的圖解。
    - **風格統一**：確保所有 Icon 的線條粗細、圓角與配色一致。
    - **模組化設計**：建立可重複使用的圖表元件庫。
- **實作產出**：一份 4-6 頁的知識型懶人包 (Carousel)。

## 雙軌實戰任務 (Dual-Track Specs)
#### **第 6 堂：懶人包與圖表 - 讓 AI 畫圖標**

- **學習主軸**：AI 生成圖標素材 + Affinity 資訊排版。
- **【主題 A：區塊鏈/AI 運作原理】**
    - **風格**：**等距視角 (Isometric)** - 2.5D、科技感、精細。
    - **內容**：如何鑄造 NFT / AI 生成流程。
...
- **【主題 B：手沖咖啡/植栽教學】**
    - **風格**：**手繪線稿 (Hand-drawn Line Art)** - 溫暖、親切、簡約。
    - **內容**：手沖咖啡步驟 / 龜背芋照顧指南。
    - **參考文案內容 (Reference Copy)**：
        - **標題**：完美手沖咖啡指南
        - **步驟一**：研磨咖啡豆 (Grind) - 中度研磨
        - **步驟二**：溫杯與潤紙 (Rinse) - 去除紙味
        - **步驟三**：悶蒸 (Bloom) - 釋放香氣
        - **步驟四**：注水 (Pour) - 穩定水流
    - **Prompt 進階三部曲 (由淺入深)**：
        1. **Lv1 (基礎)**：
