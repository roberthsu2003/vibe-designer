...
- **【主題 A：AI 科技顧問】**
    ...
    - **Prompt 進階三部曲 (由淺入深)**：
        1. **Lv1 (基礎)**：
            ```prompt
            科技背景，極簡，留白 (Tech background, minimalist, copy space)
            ```
        2. **Lv2 (中階)**：
            ```prompt
            抽象科技背景，藍色線條，簡單，適合文字的乾淨版面 (Abstract technology background, blue lines, simple, clean layout for text)
            ```
        3. **Lv3 (高階)**：
            ```prompt
            [主體] 抽象電路板線條與幾何圖形
...
- **【主題 B：花藝/甜點師】**
    ...
    - **Prompt 進階三部曲 (由淺入深)**：
        1. **Lv1 (基礎)**：
            ```prompt
            水彩背景，柔和，留白 (Watercolor background, soft, copy space)
            ```
        2. **Lv2 (中階)**：
            ```prompt
            抽象水彩背景，粉彩色調，柔和，中間乾淨 (Abstract watercolor background, pastel colors, soft, clean center)
            ```
        3. **Lv3 (高階)**：
            ```prompt
            [主體] 柔和的抽象花卉形狀
