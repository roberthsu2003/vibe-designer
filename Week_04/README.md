...
- **【主題 A：電競酒吧/未來咖啡廳】**
    ...
    - **Prompt 進階三部曲 (由淺入深)**：
        1. **Lv1 (基礎)**：
            ```prompt
            霓虹漢堡，暗色背景，留白 (Neon burger, dark background, copy space)
            ```
        2. **Lv2 (中階)**：
            ```prompt
            賽博龐克風格漢堡，發光醬汁，暗色背景，菜單空間 (Cyberpunk style burger, glowing sauce, dark background, space for menu)
            ```
        3. **Lv3 (高階)**：
            ```prompt
...
- **【主題 B：有機早午餐/慢食餐廳】**
    ...
    - **Prompt 進階三部曲 (由淺入深)**：
        1. **Lv1 (基礎)**：
            ```prompt
            酪梨吐司，白底，留白 (Avocado toast, white background, copy space)
            ```
        2. **Lv2 (中階)**：
            ```prompt
            白盤上的酪梨吐司，俯視，適合文字的乾淨背景 (Avocado toast on white plate, top view, clean background for text)
            ```
        3. **Lv3 (高階)**：
            ```prompt
            [主體] 頂級酪梨吐司搭配水波蛋
